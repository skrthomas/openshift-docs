...
* Cluster administrators can specify a custom error code response page for either 503, 404, or both error pages. The router does not reload to reflect custom error code pages updates. As a workaround, rsh into pods and run `reload-haproxy` in all router pods that serve the custom http error code pages:
